...
| `window.print()` (and `self.`/`globalThis.`) bypassed the shadow above | Shadowing the *bare* name `print` did nothing for the *qualified* form — `window` itself was still the real one, so `window.print()` still opened the dialog | `window`, `self`, `globalThis`, `top`, `parent`, `frames` are now shadowed with a Proxy that throws on any property access or call |
| Undeclared `location = …` navigated the whole app away | Forgetting `let`/`const` before a variable named `location` (plausible in a grid/coordinate problem) assigns to the real `window.location`, which **navigates away with no dialog and no way back** — worse than the print bug, since it discards the whole session, not just something dismissible | `location` (and `history`) added to the same shadow list; confirmed live that the tab no longer navigates |
| Bundled test-case expressions had zero sandboxing | `testCase.expected`/`.input` are evaluated with `new Function` too, with none of the above protection — a bad LLM generation run could in principle produce a test case that reaches a real global directly | Both evaluators now route through the same `evalLiteral()` helper, sharing the full shadow list |
